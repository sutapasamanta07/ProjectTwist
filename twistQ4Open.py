...
#Defining the quantum ciruit (the unitary for twisted Ising open chain)
qasm_str= """
OPENQASM 2.0;
include "qelib1.inc";
qreg q[4];
...
options.transpilation.initial_layout = [16,19,22,25] #Choose the layout according to the quantum-device's most efficient positions and connections
#Define observables
...
myfile=open("Q4ErrorCorrection0010RL1Hanoi.txt",'w')
myfile.write("Q0=")
...
plt.title("Error Mitigation RL=1, Initial state=[0010]_y, Device = IBM Hanoi")
plt.ylabel('$<s_y>$')
plt.xlabel('t')
plt.legend()
# #plt.show()
plt.savefig('Q4ErrorCorrection0010RL1Hanoi.pdf')
